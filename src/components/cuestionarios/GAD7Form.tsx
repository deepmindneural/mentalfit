...
import { gad7Preguntas, interpretarGAD7, generarRecomendaciones } from '../../data/cuestionarios';
...
const GAD7Form: React.FC = () => {
  ...
  const todasPreguntasRespondidas = (): boolean => {
    return respuestas.length === gad7Preguntas.length;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const interpretacion = interpretarGAD7(puntajeTotal);
    const recomendaciones = generarRecomendaciones('GAD7', puntajeTotal);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 md:p-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('cuestionarioAnsiedad')}</h2>
